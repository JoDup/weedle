...
def insTracker(conn, name, value, bpmin, bpmax, unit):
    ...
    cursor = conn.execute("SELECT * FROM W_METRIC_TRACKER WHERE NAME=? and DAY=? and HOUR=?",
    ...
    if w_mt_id == -1:
        ...
        conn.execute("INSERT INTO W_METRIC_TRACKER (NAME, VALUE, UNIT, DAY, HOUR, BEST_PRACTICE_MIN, BEST_PRACTICE_MAX) VALUES (?, ?, ?, ?, ?, ?, ?)",
                    (name, value, unit,
        ...
        cursor = conn.execute("UPDATE W_METRIC_TRACKER SET VALUE=?, BEST_PRACTICE_MIN=?, BEST_PRACTICE_MAX=? WHERE NAME=? and DAY=? and HOUR=?",
...
#    conn.execute("update w_recommendation set state='OLD' and TIME <?",(currentHour,))
    ...
    cursor = conn.execute("SELECT * FROM W_RECOMMENDATION WHERE CHECK_NAME=? and ELEMENT= ? and ACTION=? and ACTION_MINUTE=? and DAY=? and HOUR=?",
    ...
    if w_r_id == -1:
        ...
        conn.execute("INSERT INTO W_RECOMMENDATION (CHECK_NAME, ELEMENT, ACTION, ACTION_MINUTE, RECOMMENDATION, STATE, DAY, HOUR, MINUTE, PRIORITY) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
...
def recommendation(plantSize):
    ...
    conn.execute("delete from w_recommendation where date(day) < date('now','-3 days')")
